...
public class C206_CaseStudyTest {
	...
	public void setUp() throws Exception {
		...
		p1 = new Promotion(1, "Fruit Stall", "Watermelon");
		p2 = new Promotion(2, "Drink Stall", "Green Tea");
		p3 = new Promotion(3, "Western Stall", "Fish & Chips");
	...
	public void testAddNewPromotion() {
		promoList.add(p1);
		assertNotNull("Test if there is valid Account arraylist to add to", promoList);
		
		assertEquals("Test that Account arraylist size is 1", 1, promoList.size());
		
		promoList.add(p2);
		assertEquals("Test that Account arraylist size is 2", 2, promoList.size());

	}
	
	@Test
	public void testViewAllPromotion() {
		
		assertNotNull("Test if there is valid Account arraylist to view", promoList);

		String allPromo = C206_CaseStudy.retrieveAllPromotion(promoList);
		String testOutput = "";
		assertEquals("Test that the viewed AccountList is empty?", testOutput, allPromo);

		promoList.add(p1);
		allPromo = C206_CaseStudy.retrieveAllPromotion(promoList);
		testOutput = String.format("%-10s %-30s %-10s\n", 1, "Fruit Stall", "Watermelon");

		assertEquals("Test that ViewAllPromolist", testOutput, allPromo);
	}
	...
	public void testDeletePromo() {
		...
		promoList.add(p2);
		promoList.add(p3);
		assertNotNull("Test if there is valid Promotion arraylist to delete from", promoList);
		
		assertTrue("Test if an available item is ok to delete?", C206_CaseStudy.findPromo(promoList,p1.getPromoID()));
		promoList.remove(p1);
		
		assertFalse("Test if an same item is NOT ok to delete again?", C206_CaseStudy.findPromo(promoList,p1.getPromoID()));
